...
class Genealogy_Parser
{
    ...
    /**
     * Contains an array of notes, keyed by id.
     * 
     * @var    array
     * @access private
     */
    ...
    function _makeGedcomTree()
    {
      ...
      foreach($this->_GedcomTree as $elem){
        if(stristr( $elem[0], "NOTE" )){
          echo "<p>";
          $i = 0;
          $note['body'] = "";
          $note['id'] = "";
          foreach($elem as $line){
            if($i !== 0){
              $note['body'] = $note['body'] . str_replace('\n', ' ', str_replace('1 CONT', ' ', str_replace('1 CONC', ' ', trim($line))));
            } else {
              $note['id'] = intval(str_replace('@ NOTE ', ' ', str_replace('0 @H', ' ', $line)));
            }
            $i++;
          }
          $this->_Notes[$note['id']] = $note['body'];
        }
      }
    }
